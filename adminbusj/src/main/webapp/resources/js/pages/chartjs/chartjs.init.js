$(function () {
	...
	const hToplabels = hTopLine.map(item => item.bus_line);
	const hTopdata = hTopLine.map(item => item.reserved_seats);
	
	const sTopLine = JSON.parse(sTopLineJson);
	const sToplabels = sTopLine.map(item => item.bus_line);
	const sTopdata = sTopLine.map(item => item.reserved_seats);
